      ...
      <Image src={tutor.profile} alt={tutor.name} width={200} height={200} />
        ...
        <div className="flex flex-col gap-2 py-2 pl-4 pr-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex gap-2">{tutor.courses.map((item) => (<h1 className="bg-black/20 rounded-md text-gray-1 px-1 text-sm">{item}</h1>))}</div>
          </div>

          <div>
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-blue/90 font-semibold">Tutoring Type:</p>{" "}
              <div className="text-gray-1 text-sm">{tutor.tutorialType?.map((item) => (<h1>{item.label}</h1>))}</div>
            </div>
